...
uint16_t SSD1306_GetX(){
	return SSD1306.CurrentX;
}

uint16_t SSD1306_GetY(){
	return SSD1306.CurrentY;
}
void SSD1306_ResetCursor(){
	SSD1306.CurrentX = 0;
	SSD1306.CurrentY = 0;
}
...
void SSD1306_MoveRelative(uint16_t x, uint16_t y){
	SSD1306.CurrentX += x;
	SSD1306.CurrentY += y;
}
...
void SSD1306_drawBitmap(int16_t x, int16_t y, const uint8_t *bitmaps, int16_t w, int16_t h, uint16_t color) {
	int16_t i, j, byteWidth = (w + 7) / 8;

	for(j=0; j<h; j++) {
		for(int byteIndex=0; byteIndex<byteWidth;byteIndex++){
			for(i=0; i<8; i++ ) {
				if(  bitmaps[j*byteWidth+byteIndex] & 1<<8-i-1 ){
					SSD1306_DrawPixel(x+i+8*byteIndex, y+j, SSD1306_COLOR_WHITE);
				} else {
					SSD1306_DrawPixel(x+i+8*byteIndex, y+j, SSD1306_COLOR_BLACK);
				}
			}
		}
	}
